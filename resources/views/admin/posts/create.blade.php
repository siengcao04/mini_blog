...
<form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="post_type" id="post_type" class="form-select @error('post_type') is-invalid @enderror">
                            <option value="text" {{ old('post_type') === 'text' ? 'selected' : '' }}>Tin tức thường</option>
                            <option value="image" {{ old('post_type') === 'image' ? 'selected' : '' }}>Tin ảnh</option>
                            <option value="video" {{ old('post_type') === 'video' ? 'selected' : '' }}>Tin video</option>
                        </select>
                        @error('post_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" name="is_featured" value="1" id="is_featured" class="form-check-input" {{ old('is_featured') ? 'checked' : '' }}>
            ...
            <div class="card mb-3" id="video-url-card" style="display: none;">
                <div class="card-header">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (document.getElementById('post_type').value === 'video') {
            document.getElementById('video-url-card').style.display = 'block';
        }
    });
</script>
@endsection
